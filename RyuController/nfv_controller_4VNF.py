...
configFile = os.path.join(SCRIPT_PATH, "nfv_4VNF.config")
...
class nfv_controller(app_manager.RyuApp):
    ...
    def PISelection(self):
        ...
        X_1 = float(txt.split(':')[0].split('=')[1])
        X_2 = float(txt.split(':')[1].split('=')[1])
        X_3 = float(txt.split(':')[2].split('=')[1])

        vnf_selected = vnf1_interface
        ...
        if vnf_selected == vnf2_interface:
            ...
            if ran > X_2:
                ...
                vnf_selected = vnf3_interface
            else:
                self.logger.info("Error reading PI-controller generated load file! Default using vnf1 ")
                return vnf2_interface

        ## for VNF3
        if vnf_selected == vnf3_interface:
            # generate a uniform random number between 0 and 1
            ran = R.random()
